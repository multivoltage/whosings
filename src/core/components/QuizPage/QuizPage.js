...
const DEFAULT_TIME_FOR_TRACK = 4000
...
const QuizPage = ({ currentUser, game, initGame }) => {
  ...
  if (!currentUser.name.length) return <Redirect to="/" />
    ...
    <div className="quizPage">
      ...
      {game.state === 'stopped' && (
        <React.Fragment>
          ...
          <p className="quizPage--desc">
            You will have N questions and for each question you have to click on the correct artist
            according to the proposed song
          </p>
          ...
          <div className="quizPage--content">
            ...
            <span className="contentItem contentItem-input">
              <span className={`input-error ${maxSong > 0 && maxSong <= 20 ? '' : 'visible'}`}>
                check tracks number
              </span>
              <input
                type="number"
            ...
            <span className="contentItem contentItem-input">
              <span
                className={`input-error ${trackMs >= 2000 && trackMs <= 10000 ? '' : 'visible'}`}
              >
                check tracks second
              </span>
              <input
                ...
                min="2"
                max="10"
                placeholder={Math.round(DEFAULT_TIME_FOR_TRACK / 1000)}
                onChange={({ target }) => {
                  const value = Number(target.value)
            ...
            <button
              className="contentItem contentItem--fw"
              onClick={() => {
      ...
      {game.state === 'ended' && (
        <React.Fragment>
          {game.saved && (
            <h2 className="quizPage--title">
              Game ended with {game.points} points on {game.maxSong} tracks
            </h2>
          )}
